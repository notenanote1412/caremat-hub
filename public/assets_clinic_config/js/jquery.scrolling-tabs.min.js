...
;(function (e, i) {
  var a = {
    ...
    CSS_CLASSES: { SCROLL_ARROW_DISABLE: 'scrtabs-disable' },
  ...
  ;(function (w) {
    ...
    w.refreshAllElementSizes = function () {
      ...
      if (B.scrollArrowsVisible) {
        z = A.getMinPos()
        x = A.scrollToActiveTab({ isOnWindowResize: true })
        if (!x) {
    ...
    w.setElementWidths = function () {
      ...
      y.scrollArrowsCombinedWidth =
        y.$slideLeftArrow.outerWidth() + y.$slideRightArrow.outerWidth()
    ...
    w.setEventListeners = function () {
      ...
      if (z.tabClickHandler) {
        z.$tabsLiCollection
          ...
          .off(A.CLICK)
          .on(A.CLICK, z.tabClickHandler)
      }
      z.$win.smartresize(function (B) {
        y.handleWindowResize.call(y, B)
      })
      e('body').on(
        a.EVENTS.FORCE_REFRESH,
        z.elementsHandler.refreshAllElementSizes.bind(z.elementsHandler)
      )
    }
    ...
    w.setFixedContainerWidthForVisibleScrollArrows = function () {
      var x = this,
        y = x.stc
      y.$fixedContainer.width(
        y.fixedContainerWidth - y.scrollArrowsCombinedWidth
      )
    }
    w.setMovableContainerWidth = function () {
      ...
      if (y.length) {
        y.each(function A() {
          ...
          if (z.isNavPills) {
            B =
              parseInt(C.css('margin-left'), 10) +
              parseInt(C.css('margin-right'), 10)
          }
  ...
  ;(function (w) {
    w.continueSlideMovableContainerLeft = function () {
      ...
      setTimeout(function () {
        if (
          ...
          !x.$slideLeftArrow.data(a.DATA_KEY_IS_MOUSEDOWN)
        ) {
          return
        }
    ...
    w.continueSlideMovableContainerRight = function () {
      ...
      setTimeout(function () {
        if (
          ...
          !x.$slideRightArrow.data(a.DATA_KEY_IS_MOUSEDOWN)
        ) {
          return
        }
    ...
    w.decrementMovableContainerLeftPos = function (x) {
      ...
      y.movableContainerLeftPos -=
        y.fixedContainerWidth / a.SCROLL_OFFSET_FRACTION
    ...
    w.getMinPos = function () {
      var y = this,
        x = y.stc
      return x.scrollArrowsVisible
        ? x.fixedContainerWidth -
            x.movableContainerWidth -
            x.scrollArrowsCombinedWidth
        : 0
    }
    w.getMovableContainerCssLeftVal = function () {
      var y = this,
        x = y.stc
      return x.movableContainerLeftPos === 0
        ? '0'
        : x.movableContainerLeftPos + 'px'
    }
    ...
    w.setMovableContainerLeftPosToTabEdge = function (x) {
      ...
      y.$tabsLiCollection.each(function (D) {
        ...
        if (A > B) {
          y.movableContainerLeftPos =
            x === a.SLIDE_DIRECTION.RIGHT ? -(A - C) : -A
          return false
        }
      })
    }
    w.slideMovableContainerToLeftPos = function () {
      ...
      y.$movableContainer.stop().animate({ left: A }, 'slow', function B() {
        var C = z.getMinPos()
  ...
  var m = (function () {
    ...
    function B(D, G, F) {
      var E = e('<div role="tabpanel" class="tab-pane"></div>')
        .attr('id', D[G.paneId])
        .html(D[G.content])
      if (F && D[G.active]) {
        E.addClass('active')
  ...
  var r = (function () {
    ...
    function w(A, E) {
      ...
      while (B === -1 && C >= 0) {
        if (
          ((C = E + ++D) <= F && !A.eq(C).hasClass('disabled')) ||
          ((C = E - D) >= 0 && !A.eq(C).hasClass('disabled'))
        ) {
          B = C
        }
      }
    ...
    function x(C, B, A) {
      C.data({ tab: e.extend({}, B[A]), index: A })
    }
  })()
  function k(A, y, B) {
    var C = y.tabs,
  ...
  function b() {
    ...
    x.scroller
      .find('[data-' + a.DATA_KEY_DDMENU_MODIFIED + ']')
      .css({ display: '', left: '', top: '' })
      .off(a.EVENTS.CLICK)
      .removeAttr('data-' + a.DATA_KEY_DDMENU_MODIFIED)
  ...
  function s(z) {
    ...
    function w(G) {
      var H = e(G.target),
        ...
        C,
        B
      y = H.find('.dropdown-menu').attr(
        'data-' + a.DATA_KEY_DDMENU_MODIFIED,
        true
      )
      if (F[0] !== H[0]) {
      ...
      y.css({ display: 'block', top: A.top + H.outerHeight() - 2, left: B })
      function E(K) {
        var M = e(this),
          J = M.parent('li'),
    ...
    init: function (w) {
      ...
      if (!x.tabs) {
        return z.each(function (B) {
          var A = { isWrapperOnly: true },
            C = e(this).data({ scrtabs: A }),
            D =
              B < y
                ? null
                : function () {
